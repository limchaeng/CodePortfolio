...
namespace UMF.Unity.EditorUtil
{
	public static class InspectorUtil
	{
		...
		public static DynamicBtnListBoxData DrawDynamicButtonListBox( List<DynamicBtnListBoxData> data_list, eDynamicBtnListBoxFlag flags, Color selected_color, float padding = 4f )
		{
			...
			foreach( DynamicBtnListBoxData data in data_list )
			{
				...
				if( ( flags & eDynamicBtnListBoxFlag.DropDownBtn ) != 0 )
				{
					clicked = EditorGUILayout.DropdownButton( gui_content, FocusType.Passive, GUILayout.Width( size.x ) );
				}
				else
				{
					clicked = GUILayout.Button( gui_content, GUILayout.Width( size.x ) );
				}
		...
		//------------------------------------------------------------------------
		public static void Draw_PrefabControl( GameObject game_object, System.Action saved_callback )
		{
			Draw_PrefabControl( game_object, Application.dataPath, saved_callback, null );
		}
		public static void Draw_PrefabControl( GameObject game_object, System.Action saved_callback, System.Func<bool, GameObject> valid_checker )
		{
			Draw_PrefabControl( game_object, Application.dataPath, saved_callback, null );
		}
		public static void Draw_PrefabControl( GameObject game_object, string save_root_path, System.Action saved_callback, System.Func<bool, GameObject> valid_checker )
		{
			...
			if( Application.isPlaying == false && game_object != null )
			{
				...
				else
				{
					...
					if( GUILayout.Button( "Prefab Apply", GUILayout.Height( 20f ) ) )
					{
						...
						if( is_valid )
						{
							if( EditorUtility.DisplayDialog( "Q", string.Format( "Prefab ({0}) Apply?", game_object.name ), "ok", "cancel" ) )
							{
	...
	public class ReadOnlyAttributeDrawer : PropertyDrawer
	{
		...
		{
			GUI.enabled = Application.isPlaying == false && ( (ReadOnlyAttribute)attribute ).runtimeOnly;
