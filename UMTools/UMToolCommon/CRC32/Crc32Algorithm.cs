...
namespace Force.Crc32
{
	...
	public class Crc32Algorithm : HashAlgorithm
	{
		...
		public static uint ComputeCRC( System.IO.Stream stream, int buff_len = 4096 )
		{
			byte[] buffer = new byte[buff_len];
			uint curr_crc = 0;
			int byte_read = 0;
			while( ( byte_read = stream.Read( buffer, 0, buff_len ) ) > 0 )
			{
				curr_crc = Append( curr_crc, buffer, 0, byte_read );
			}
