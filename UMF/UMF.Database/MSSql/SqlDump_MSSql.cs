...
{
	public class SqlDump_MSSql
	{
		//------------------------------------------------------------------------		
		public static string GetCommandDump( string prefix, SqlCommand sqc )
		{
			try
			{
		...
		{
			sbCommandText.AppendLine( string.Format( " {0};", param.TypeName ) );
			if( param.Value != null )
			{
		...
		//------------------------------------------------------------------------		
		// for DataTable
		private static void logStructuredParameter_forDataTable( SqlParameter param, StringBuilder sbCommandText )
		{
