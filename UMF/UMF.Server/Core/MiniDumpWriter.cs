...
namespace UMF.Server
{
	public class MinidumpWriter
	{
		...
		static System.Action<string> LOG = null;

		public static bool MakeDump( string dumpFilePath, int processId, MiniDumpType _type_flags )
		{
			SetDumpPrivileges();
		...
		private static void SetDumpPrivileges()
		{
			...
			{
				if( LOG != null )
					LOG( string.Format( "OpenProcessToken() failed, error = {0} . SeDebugPrivilege is not available", Marshal.GetLastWin32Error() ) );
				return;
			}
			...
			{
				if( LOG != null )
					LOG( string.Format( "LookupPrivilegeValue() failed, error = {0} .SeDebugPrivilege is not available", Marshal.GetLastWin32Error() ) );
				CloseHandle( hToken );
				return;
			}
